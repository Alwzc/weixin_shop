...
class WechatPlatformController extends Controller
{
    ...
    {
        $redirect_uri = urlencode(\Yii::$app->urlManager->createAbsoluteUrl(['wechat-platform/bind-user', 'store_id' => $this->store->id]));
    ...
    private function bindUser($unionid, $openid)
    {
        ...
        if (!$user) {
            throw new \Exception('小程序未绑定开放平台或您未在小程序上登录，若已登陆，请删除小程序重新进入小程序会员中心再点击此菜单。unionid=' . $unionid);//P_MOD
        }
