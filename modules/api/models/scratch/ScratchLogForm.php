...
class ScratchLogForm extends ApiModel
{
    ...
    public function log()
    {
        $list = ScratchLog::find()
            ...
            }])->limit(2)->orderBy('create_time DESC,id DESC')->all();
    ...
    public function search()
    {
        ...
        $list = $form->limit($pagination->limit)->offset($pagination->offset)->orderBy('create_time DESC,id DESC')->all();
        foreach($list as &$v){
    ...
    protected function simplifyData($data){
        foreach($data as $key=>$val){
            ...
            if($val->user){
                $newData[$key]['user'] = $val->user->attributes['nickname'];
            }
        }
        return $newData;
    }
