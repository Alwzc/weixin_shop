...
class ShareQrcodeForm extends ApiModel
{
    ...
    public function ms_goods_qrcode()
    {
        ...
        $editor->text($goods_qrcode, $time, 21, 170, 150, new Color('#ffffff'), $font_path, 0);

        //加商品价格
        // $editor->text($goods_qrcode, '￥' . (empty($miaosha_price) ? $miaosha_price : $goods->original_price), 20, 30, 962, new Color('#ff4544'), $font_path, 0);

        // 商品价格钱币符
        $editor->text($goods_qrcode, '￥', 20, 30, 1002, new Color('#ff5c5c'), $font_path, 0);
        //加商品价格
        $editor->text($goods_qrcode, $miaosha_price, 34, 55, 990, new Color('#ff5c5c'), $font_path, 0);

        //加商城名称
        $editor->text($goods_qrcode, $store->name, 20, 40, 1200, new Color('#888888'), $font_path, 0);
    ...
    public function pt_goods_qrcode()
    {
        ...
        $editor->text($goods_qrcode, '￥', 20, 30, 1002, new Color('#ff5c5c'), $font_path, 0);
        //加商品价格
        $editor->text($goods_qrcode, $goods->price, 34, 55, 990, new Color('#ff5c5c'), $font_path, 0);
...
//        var_dump($namewitch[2]);die();
        ...
        // 商品价格钱币符
        $editor->text($goods_qrcode, '预约金额 ￥', 20, 30, 962, new Color('#ff5c5c'), $font_path, 0);
        //加商品价格
        $editor->text($goods_qrcode, $goods->price, 29, 170, 950, new Color('#ff5c5c'), $font_path, 0);
