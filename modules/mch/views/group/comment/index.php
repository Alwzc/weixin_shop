...
<div class="panel mb-3">
    <div class="panel-header">        
        ...
        <ul class="nav nav-right">
            <li class="nav-item">
                <a class="nav-link" href="<?= $urlManager->createUrl(['mch/group/comment/edit']) ?>">添加虚拟评价</a>
            </li>
        </ul>
        ...
        <table class="table table-bordered bg-white">
            ...
            <?php foreach ($list as $index => $item): ?>
                <tr>
                    ...
                    <td class="nowrap">
                         <a <?= $item['is_virtual'] ?false : hidden ?> class="btn btn-sm btn-warning"
                           href="<?= $urlManager->createUrl(['mch/group/comment/edit', 'id' => $item['id'], 'status' => 1]) ?>">修改</a>
                        <?php if ($item['is_hide'] == 0): ?>
                            <a class="btn btn-sm btn-primary hide-status-btn"
                               href="<?= $urlManager->createUrl(['mch/group/comment/hide-status', 'id' => $item['id'], 'status' => 1]) ?>">隐藏</a>
                        <?php else: ?>
                            <a class="btn btn-sm btn-primary hide-status-btn"
                               href="<?= $urlManager->createUrl(['mch/group/comment/hide-status', 'id' => $item['id'], 'status' => 0]) ?>">显示</a>
                        <?php endif; ?>
                        <a class="btn btn-sm btn-danger delete-status-btn"
                           href="<?= $urlManager->createUrl(['mch/group/comment/delete-status', 'id' => $item['id'], 'status' => 1]) ?>">删除</a>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
