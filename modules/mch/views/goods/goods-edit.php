        ...
        <form class="auto-form" method="post" return="<?= $returnUrl ?>">
            ...
            <div class="head">
                <div class="head-content" flex="dir:left">
                    ...
                    <!--                    <a flex="cross:center" class="head-step" href="#step8">会员折扣</a>-->
                    <?php if (count($plugins) > 0): ?>
                        <a flex="cross:center" class="head-step" href="#step9">砍价设置</a>
                    <?php else: ?>
            ...
            <div class="step-block" flex="dir:left box:first">
                ...
                <div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-9">
                            <input class="form-control short-row" type="number" name="model[confine_count]"
                        ...
                        <div class="col-9">
                            <label class="radio-label">
                                <input <?= $goods['is_level'] == 0 ? 'checked' : null ?>
                                    value="0" name="model[is_level]" type="radio" class="custom-control-input">
                                <span class="label-icon"></span>
                                <span class="label-text">关闭</span>
                            </label>
                            <label class="radio-label">
                                <input <?= $goods['is_level'] == 1 ? 'checked' : null ?>
                                    value="1" name="model[is_level]" type="radio" class="custom-control-input">
                                <span class="label-icon"></span>
...
<script>
    ...
    var page = new Vue({
        ...
        data: {
            sub_cat_list: [],
            attr_group_list: JSON.parse('<?=json_encode($goods->getAttrData())?>', true),//可选规格数据
            attr_group_count: JSON.parse('<?=json_encode($goods->getAttrData())?>', true).length,
            checked_attr_list: JSON.parse('<?=json_encode($goods->getCheckedAttrData())?>', true),//已选规格数据
...
//                    $("input[name='model[cover_pic]']").val(res.data.cover_pic);
                    ...
                    $("input[name='model[use_attr]']").val(0);
