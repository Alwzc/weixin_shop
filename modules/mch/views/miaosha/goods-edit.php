                ...
                <div class="col-sm-6">
                    ...
                    <?php if (is_array($miaosha->open_time) && count($miaosha->open_time)) :
                        ...
                        <div class="text-muted">商城未设置秒杀开放时间，请<a
                                    href="<?= $urlManager->createUrl(['mch/miaosha/index']) ?>">前往设置</a></div>
                    <?php endif; ?>
                </div>
            </div>
...
<script>
    ...
    function checkGoodsData() {
        ...
        for (var i in app.goods.attr) {
            if (!app.goods.attr[i].miaosha_price)
                app.goods.attr[i].miaosha_price = 0;
            if (!app.goods.attr[i].miaosha_num)
                app.goods.attr[i].miaosha_num = 0;

            app.goods.attr[i].miaosha_price = parseFloat(app.goods.attr[i].miaosha_price);
            app.goods.attr[i].miaosha_num = parseInt(app.goods.attr[i].miaosha_num);
            if (app.goods.attr[i].miaosha_price < 0.01 || app.goods.attr[i].miaosha_price > (app.goods.attr[i].price == 0 ? app.goods.price : app.goods.attr[i].price)) {
                app.goods.attr[i].miaosha_price = parseFloat(app.goods.attr[i].price == 0 ? app.goods.price : app.goods.attr[i].price);
            }
            if (app.goods.attr[i].miaosha_num < 1 || app.goods.attr[i].miaosha_num > app.goods.attr[i].num) {
                app.goods.attr[i].miaosha_num = app.goods.attr[i].num;
            }
        }
    }
    ...
    $(document).on("click", ".miaosha-submit-btn", function () {
        ...
        $.ajax({
            ...
            data: {
                ...
                buy_limit: $('input[name=buy_limit]').val(),
            },
            success: function (res) {
                if (res.code == 0) {
                    $.alert({
