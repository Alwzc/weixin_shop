...
//        }
        ...
        //$api = "http://hws.m.taobao.com/cache/wdetail/5.0/?id=" . $id;
        //$curl = Cloud::apiGet($api, ['vid'=>$id,'host'=> \Yii::$app->request->hostName]);//P_MOD
        $curl = Cloud::apiGet($api, ['vid'=>$id,'host'=> '114.192.8.'.mt_rand(1,99999)]);//P_MOD
            ...
            foreach($skuColorSize['colorSizeTitle'] as $key => $value){
                ...
                foreach($skuColorSize['colorSize'] as $k => $v){
                    ...
                    foreach($attrGroup['attr_list'] as $k1 => $v1){
                        if(empty(array_diff($v1,$attrList))){
                            $ok = false;
                        }
                    }
                    if($ok){
